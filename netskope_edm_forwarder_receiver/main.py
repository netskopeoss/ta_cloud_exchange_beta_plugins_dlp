    ...
    def get_fields(self, name: str, configuration: dict):
        ...
        if name == "configuration":
            ...
            if "auth_method" in protocol_configuration and protocol_configuration[
                "auth_method"
            ] in ["basic_auth", "sso_auth", "secret_token_auth"]:
                return NETSKOPE_CE_AUTH_TYPE_FIELDS[
                    protocol_configuration["auth_method"]
                ]
            else:
                raise ValueError(
                    "Protocol is required field & It should be either 'basic_auth','sso_auth' or 'secret_token_auth'"
                )
        else:
            raise NotImplementedError()
